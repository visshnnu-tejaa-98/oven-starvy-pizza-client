...
const AdminRegister = () => {
	...
	const postData = async () => {
		...
		if (name && email && password) {
			...
			console.log(data);
			let req = await fetch(`${BACKEND_ENDPOINT}/admin/register`, {
				method: 'POST',
				headers: {
			...
			if (registerStatus.message === 'User Already Exists') {
				...
				window.location.href = `${FRONTEND_ENDPOINT}/admin/register`;
			} else {
				window.location.href = `${FRONTEND_ENDPOINT}/admin/login`;
			}
		} else {
